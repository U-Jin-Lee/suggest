...
class DailySuggestCollector:
    def __init__(self,
        ...
        self.make_result_folder()
        self.hdfs_result_path = f"/user/ds/wordpopcorn/ko/daily/google_suggest_for_trend/{self.date[:4]}/{self.date[:6]}/{self.date}/{self.now}"
        self.hdfs_result_path_target = f"/user/ds/wordpopcorn/ko/daily/google_suggest_for_trend_target/{self.date[:4]}/{self.date[:6]}/{self.date}"
    
    def make_result_folder(self):
    ...
    def target_suggest_job(self):
        ...
        for kw in target_keywords:
            ...
            gzip(result_file_path)
        try:
            self.hdfs.upload(source=self.local_result_path, dest=self.hdfs_result_path_target, overwrite=True)
            remove_folder(self.local_result_path)
            self.make_result_folder()
        except Exception as e:
            print(f"error from upload to hdfs : {e}")
        ds_daily_dbgout(prefix_message + f"jobid : {self.jobid}\n" + f"타겟 키워드 {len(target_keywords)}개 서제스트 수집 완료 (hdfs dest : {self.hdfs_result_path_target})")
        
    def basic_suggest_job(self):
        result_file_path = self.get_suggest()
        gzip_file_path = gzip(result_file_path)
        try:
            self.hdfs.upload(source=gzip_file_path, dest=self.hdfs_result_path, overwrite=True)
            remove_folder(gzip_file_path)
        except Exception as e:
            print(f"error from upload to hdfs : {e}")
            ds_daily_dbgout_error(prefix_message + f"jobid : {self.jobid}\n" + f"기본 서제스트 hdfs 업로드 실패 (error msg : {e})")
        ds_daily_dbgout(prefix_message + f"jobid : {self.jobid}\n" + f"기본 서제스트 수집 완료 (hdfs dest : {self.hdfs_result_path})")        
